...
def search2():
    reps = 5
    with open('rbn_parameters2.txt', 'w') as f:
        f.write('I\tK\t%\n')
        for i in range(20):
            for k in range(0, 70, 5):
                ...
                for _ in range(reps):
                    ans = subprocess.check_output(
                            'python hd_rbn.py --average --i ' + str(i) + 
                            ' --bits 2 --random --dev 2 --k ' + str(k/10),
                            shell=True)
                    total += float(ans.decode('utf8').split(' ')[-2])
                    print('\t%f' %(total))
                total = total / reps    
...
def plot2():
    success = np.zeros(shape=(14, 20))
    with open('rbn_parameters2.txt', 'r') as f:
        f.readline()
        for line in f:
            currentline = list(map(float, line.split('\t')))
            success[int(currentline[1]/5)][int(currentline[0])] = \
                    currentline[2]

    x = np.linspace(0, 19, 20)
    y = np.linspace(0, 6.5, 14)
    X, Y = np.meshgrid(x, y)
    Z = success
    fig = plt.figure()
    ax = plt.axes(projection='3d')
